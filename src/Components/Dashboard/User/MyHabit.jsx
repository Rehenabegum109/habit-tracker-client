...
const MyHabit = () => {
  ...
  const fetchHabits = async () => {
    try {
      const res = await axiosSecure.get(`/habits?userEmail=${user.email}`);
      setHabits(res.data);
      setLoading(false);
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async (habitId) => {
    ...
    try {
      await axiosSecure.delete(`/habits/${habitId}`);
  ...
  const handleMarkComplete = async (habit) => {
    try {
      ...
      await axiosSecure.patch(`/habits/${habit._id}`, { currentStreak: newStreak });
